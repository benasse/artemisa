...
#                print self.Via[i]
    ...
    def gl(self, strIP):
        ...
        if strIP == "": # Means that the address wasn't able to be resolved
            ...
            self.add_results("| |")
            self.add_results("| | Cannot resolve DNS!")
            if self.verbose == True:
                self.add_results("| |")
                self.add_results("| | (Verbose) Lambda_to_parent_gl: (0.2,0.8)")
            ...
            self.add_results("| + Geographical analysis")
            self.add_results("| |")
            self.add_results("| |Cannot resolve DNS since dig command is not installed.")
            if self.verbose == True:
                self.add_results("| |")
                self.add_results("| | (Verbose) Lambda_to_parent_gl: (1,1)")
    ...
    def gethops(self, strIP):
        ...
        if strIP == "": # The IP wasn't able to be resolved
            self.add_results("| Cannot get number of hops from " + strIP + ". 99 hops are assumed.")
            return 99 # An infinite value of hops
        elif strIP == -1:
            self.add_results("| Cannot get number of hops from " + strIP + " since there was an error in name resolution. 99 hops are assumed.")
            return 99 # An infinite value of hops
        
        try:
            Process = Popen("traceroute -4 -I -N 1 -n -q 1 -w 2 " + strIP + " | wc -l", shell=True, stdout=PIPE)
            Process.wait()
            hops = (int(Process.communicate()[0].strip()) - 1)
            if hops < 0: 
                return 0
            else:
                return hops
            ...
            self.add_results("| Cannot get number of hops from " + strIP + " since traceroute command is not installed. 99 hops are assumed.")
            return 99 # An infinite value of hops
