...
class InferenceAnalysis(PrintClass, log, GetTimeClass, CallData):
    ...
    def Start(self):
        ...
        lambda_to_parent_trust_1 = self.TrustAnalysis(self.From_IP)
        
        lambda_to_parent_trust_2 = []
        
        for i in range(len(self.Via)):
            self.add_results("")
            self.add_results("+ Analysis of Via " + str(i) + ": " + self.Via[i])
            
            lambda_to_parent_trust_2.append(self.TrustAnalysis(self.Via[i]))

        self.add_results("")
        self.add_results("+ Analysis of Contact IP: " + self.Contact_IP)
        
        lambda_to_parent_trust_3 = self.TrustAnalysis(self.Contact_IP)

        self.add_results("")
        self.add_results("+ Analysis of Connection IP: " + self.Connection)
        
        lambda_to_parent_trust_4 = self.TrustAnalysis(self.Connection)
        
        self.add_results("")
        self.add_results("+ Analysis of Owner IP: " + self.Owner)
        
        lambda_to_parent_trust_5 = self.TrustAnalysis(self.Owner)

        # Fuse all lambda_to_parent_trust_N
        
        if len(lambda_to_parent_trust_2) == 0:
            lambda_to_parent_trust = lambda_to_parent_trust_1[0] * lambda_to_parent_trust_3[0] * lambda_to_parent_trust_4[0] * lambda_to_parent_trust_5[0], lambda_to_parent_trust_1[1] * lambda_to_parent_trust_3[1] * lambda_to_parent_trust_4[1] * lambda_to_parent_trust_5[1], lambda_to_parent_trust_1[2] * lambda_to_parent_trust_3[2] * lambda_to_parent_trust_4[2] * lambda_to_parent_trust_5[2]
        else:
            lambda_to_parent_trust = lambda_to_parent_trust_1[0] * lambda_to_parent_trust_3[0] * lambda_to_parent_trust_4[0] * lambda_to_parent_trust_5[0], lambda_to_parent_trust_1[1] * lambda_to_parent_trust_3[1] * lambda_to_parent_trust_4[1] * lambda_to_parent_trust_5[1], lambda_to_parent_trust_1[2] * lambda_to_parent_trust_3[2] * lambda_to_parent_trust_4[2] * lambda_to_parent_trust_5[2]
            
            for i in range(len(lambda_to_parent_trust_2)):
                lambda_to_parent_trust = lambda_to_parent_trust[0] * lambda_to_parent_trust_2[i][0], lambda_to_parent_trust[1] * lambda_to_parent_trust_2[i][1], lambda_to_parent_trust[2] * lambda_to_parent_trust_2[i][2],
                
            
        
        beta = (1/(lambda_to_parent_trust[0] + lambda_to_parent_trust[1] + lambda_to_parent_trust[2])) # Normalization constant.
        ...
        lambda_nature = (lambda_to_parent_trust[0] * lambda_to_parent_to[0] * lambda_to_parent_dispersion[0] * lambda_to_parent_fingerprint[0], lambda_to_parent_trust[1] * lambda_to_parent_to[1] * lambda_to_parent_dispersion[1] * lambda_to_parent_fingerprint[1], lambda_to_parent_trust[2] * lambda_to_parent_to[2] * lambda_to_parent_dispersion[2] * lambda_to_parent_fingerprint[2])
        ...
        if self.verbose == True:
            ...
            self.add_results("| (Verbose) Lambda_to_parent_fingerprint: " + str(round(lambda_to_parent_fingerprint[0],4)) + ", " + str(round(lambda_to_parent_fingerprint[1],4)) + ", " + str(round(lambda_to_parent_fingerprint[2],4)))
        ...
        self.NormalProb = round(bel_nature[0],6)
        self.SuspiciousProb = round(bel_nature[1],6)
        self.CraftedProb = round(bel_nature[2],6)
    ...
    def TrustAnalysis(self, strIP):
        ...
        lambda_trust = (lambda_to_parent_historical[0]*lambda_to_parent_whois[0]*lambda_to_parent_gl[0]*lambda_to_parent_ipreliability[0]*lambda_to_parent_dns[0],lambda_to_parent_historical[1]*lambda_to_parent_whois[1]*lambda_to_parent_gl[1]*lambda_to_parent_ipreliability[1]*lambda_to_parent_dns[1])
    ...
    def GetCallData(self):
        ...
        self.From_IP = GetIPfromSIP(GetSIPHeader("To",self.Message))
        self.From_Extension = GetExtensionfromSIP(GetSIPHeader("To",self.Message))
        
        self.Contact_IP = GetIPfromSIP(GetSIPHeader("To",self.Message))
        self.Contact_Extension = GetExtensionfromSIP(GetSIPHeader("To",self.Message))
        ...
        self.RecordRoute = GetSIPHeader("Record-Route",self.Message)
    ...
    def whois(self, strIP):
        ...
        # --------------|----------------|-----------------------
        #   Trusted     |      0.55      |      0.45
        #   Distrusted  |      0.45      |      0.55
        #

        CPT_matrix = [(0.55,0.45),(0.45,0.55)]
    ...
    def gl(self, strIP):
        ...
        ip2c = ip2country.IP2Country(verbose=False)
        ...
        self.add_results("| |")
        self.add_results("| | Location: " + strIP2C)
    ...
    def dns(self, strIP):
        ...
        if strIP != "":
            lambda_to_parent = (0.8,0.2)
        else:
            lambda_to_parent = (0.2,0.8)
    ...
    def fingerprint(self):
        ...
        for line in File:
            ...
            if line.find("#") == -1:
                if line.find("=") != -1:
                    ...
                    iReliability = float(line.split("=")[1])
        ...
        # x = iReliability
        #
        #               |     x < 0.3    |  0.3 < x < 0.7 |    x > 0.7     |
        # --------------|----------------|----------------------------------
        #   Normal      |      0.05      |      0.15      |      0.8       |
        #   Suspicious  |      0.5       |      0.4       |      0.1       |
        #   Crafted     |      0.6       |      0.3       |      0.1       |
        #
        
        if iReliability <= 0.3:
            lambda_to_parent = (0.05,0.5,0.6)
        elif iReliability > 0.3 and iReliability < 0.7:
            lambda_to_parent = (0.15,0.4,0.3)
        elif iReliability >= 0.7:
            lambda_to_parent = (0.8,0.1,0.1)     
    ...
    def historical(self, strIP):
        ...
        if os.path.isfile("./historical/database.txt") == True:
            ...
            #
            #               |  nMatches = 0  | 3 < nMatches < 6 | nMatches > 6  
            # --------------|----------------|------------------|------------------
